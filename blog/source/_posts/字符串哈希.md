...
void init_hash(string &str, int n){
    ...
    for(int i = n-1, val = 0; i >=0; --i){
        stufix_hash[i] = val = ((ll)val * B + str[i]) % M;
    }
}
...
int segment_stufix_hash(int i, int j, int n){
    int ans = stufix_hash[i];
    if(j != n-1) ans = (ans - (ll)stufix_hash[j + 1] * base_power[j-i+1]) % M;
    if(ans < 0) ans += M; 
    return ans;
}

bool is_palindrome(int i, int j, int n){
    return segment_prefix_hash(i, j) == segment_stufix_hash(i, j, n);
}
